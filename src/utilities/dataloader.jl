...
function load_data(JSON_file)
    ...
    MinRunCapacity, MaxRunCapacity, RampUp, RampDown, UpTime, DownTime, StartUp, ShutDown, NoLoadConsumption, FixedCost, MarginalCost = [], [], [], [], [], [], [], [], [], [], []
...
function load_rts_data(JSON_file)
    ...
    for (sym, gen) in Gen
        push!(MinRunCapacity, gen.power_output_minimum)
        push!(MaxRunCapacity, gen.power_output_maximum)
        push!(RampUp, gen.ramp_up_limit / 3.)
        push!(RampDown, gen.ramp_down_limit / 3.)
        push!(UpTime, gen.time_up_minimum)
        push!(DownTime, gen.time_down_minimum)
    ...
    instance = Instance(
        LostLoad=3000.0,
        Load=Array{Float64}(df.demand) ./ 2,
        ThermalGen=Gen
    )
    return instance
end

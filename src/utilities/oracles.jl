...
function Matching(instance)
    ...
    @constraint(model, ConstrLogical[gen=1:NbGen, t=1:T+1], Varu[gen, t] - Varu[gen, t - 1] == Varv[gen, t] - Varw[gen, t])
    @constraint(model, ConstrMinUpTime[gen=1:NbGen, t=UpTime[gen]:T+1], sum(Varv[gen, i]  for i=t-UpTime[gen]+1:t) <= Varu[gen, t])
    @constraint(model, ConstrMinDownTime[gen=1:NbGen, t=DownTime[gen]:T+1], sum(Varw[gen, i]  for i=t-DownTime[gen]+1:t) <= (1 - Varu[gen, t]))
    @constraint(model, ConstrGenLimits1[gen=1:NbGen, t=0:T+1], MinRunCapacity[gen] * Varu[gen, t] <= Varp[gen, t])
    @constraint(model, ConstrGenLimits2[gen=1:NbGen, t=0:T+1], Varp[gen, t] <= Varpbar[gen, t])
    @constraint(model, ConstrGenLimits3[gen=1:NbGen, t=0:T+1], Varpbar[gen, t] <= MaxRunCapacity[gen] * Varu[gen, t])
...
function ExtendedFormulation(instance)
    ...
    @variable(model, Varpbar[g=1:NbGen, i=1:NbIntervalsGen[g], t=0:T+1], lower_bound = 0)
    ...
    @constraint(model, MinOutput[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]:B[g, i]], - Varp[g, i, t] <= - Vargamma[g, i] * MinRunCapacity[g])
    @constraint(model, MaxOutput[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]:B[g, i]], Varp[g, i, t] - Varpbar[g, i, t] <= 0)

    @constraint(model, UpperBoundOnPbar1[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]:B[g, i]], Varpbar[g, i, t] <= Vargamma[g, i] * MaxRunCapacity[g])
    @constraint(model, UpperBoundOnPbar2[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]:B[g, i]], Varpbar[g, i, t] <= Vargamma[g, i] * (StartUp[g] + (t - A[g, i]) * RampUp[g]))
    @constraint(model, UpperBoundOnPbar3[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]:B[g, i]], Varpbar[g, i, t] <= Vargamma[g, i] * (ShutDown[g] + (B[g, i] - t) * RampDown[g]))

    @constraint(model, LimitPowerJumpsUp1[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]+1:B[g, i]], Varpbar[g, i, t] - Varp[g, i, t - 1] <= Vargamma[g, i] * RampUp[g])
    @constraint(model, LimitPowerJumpsUp2[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]+1:B[g, i]], Varpbar[g, i, t] - Varp[g, i, t - 1] <= Vargamma[g, i] * (ShutDown[g] + (B[g, i] - t) * RampDown[g] - MinRunCapacity[g]))

    @constraint(model, LimitPowerJumpsDown1[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]+1:B[g, i]], Varpbar[g, i, t - 1] - Varp[g, i, t] <= Vargamma[g, i] * RampDown[g])
    @constraint(model, LimitPowerJumpsDown2[g=1:NbGen, i=1:NbIntervalsGen[g], t=A[g, i]+1:B[g, i]], Varpbar[g, i, t - 1] - Varp[g, i, t] <= Vargamma[g, i] * (StartUp[g] + (t - A[g, i]) * RampUp[g] - MinRunCapacity[g]))

    # Packing Dispatch Polytope
    @constraint(model, MinDownTime[g=1:NbGen, t=1:T], sum(Vargamma[g, i] for i=1:NbIntervalsGen[g] if (A[g, i] <= t && t <= B[g, i] + DownTime[g])) <= 1)
    @constraint(model, ProdTime[g=1:NbGen, t=1:T], VarpTime[g, t] == sum(Varp[g, i, t] for i=1:NbIntervalsGen[g]))
    @constraint(model, ProdBarTime[g=1:NbGen, t=1:T], VarpbarTime[g, t] == sum(Varp[g, i, t] for i=1:NbIntervalsGen[g]))

    @constraint(model, CommitStatus[g=1:NbGen, t=0:T+1], sum(Vargamma[g, i] for i=1:NbIntervalsGen[g] if (A[g, i] <= t && t <= B[g, i])) == Varu[g, t])
    @constraint(model, StartUpStatus[g=1:NbGen, t=0:T+1], sum(Vargamma[g, i] for i=1:NbIntervalsGen[g] if (A[g, i] == t)) == Varv[g, t])
    @constraint(model, ShutDownStatus[g=1:NbGen, t=0:T+1], sum(Vargamma[g, i] for i=1:NbIntervalsGen[g] if (B[g, i] + 1 == t)) == Varw[g, t])
...
function LP_Relaxation(instance)
    ...
    prices = ProjBox(dual.(loads), PCD, PCU)
    return prices
end
...
function LagrangianFunction(instance, prices)
    ...
    return LagrangianSolution(objective_value(model), Varu.data, Varv.data, Varw.data, Varp.data, Varpbar.data, VarL)
end

function oracle(instance, prices)
    MinRunCapacity = instance.ThermalGen.MinRunCapacity
    MaxRunCapacity = instance.ThermalGen.MaxRunCapacity
    RampUp = instance.ThermalGen.RampUp
    RampDown = instance.ThermalGen.RampDown
    ...
    Varp, VarL = value.(Varp).data, value.(VarL)
    ObjOracle = objective_value(model)
    GradOracle = Array([VarL[t] - sum(Varp[gen,t + 1] for gen=1:NbGen) for t=1:T])
    return ObjOracle, GradOracle
end

function fast_oracle(instance, prices)
    MinRunCapacity = instance.ThermalGen.MinRunCapacity
    MaxRunCapacity = instance.ThermalGen.MaxRunCapacity
    RampUp = instance.ThermalGen.RampUp
    RampDown = instance.ThermalGen.RampDown
...
function super_fast_oracle(instance, prices)
    MinRunCapacity = instance.ThermalGen.MinRunCapacity
    MaxRunCapacity = instance.ThermalGen.MaxRunCapacity
    RampUp = instance.ThermalGen.RampUp
    RampDown = instance.ThermalGen.RampDown
...
function smooth_oracle(instance, prices, smoothing_parameter)
    MinRunCapacity = instance.ThermalGen.MinRunCapacity
    MaxRunCapacity = instance.ThermalGen.MaxRunCapacity
    RampUp = instance.ThermalGen.RampUp
    RampDown = instance.ThermalGen.RampDown

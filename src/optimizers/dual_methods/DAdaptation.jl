        ...
        push!(
            ...
            maximum([
                (
                    ...
                    sum(Gamma[i] * (D[i]^2) * norm(grad_iterates[i])^2 for i = 1:k)
                ) / (2 * norm(S[k+1])),
                D[k],
            ]),
        )
...
function tDAdaptation(
    ...
    while time_vector[end] <= budget
        ...
        it_time = @elapsed begin
            ...
            push!(
                iterates,
                Utilitaries.ProjBox(iterates[1] - Gamma[k+1] * S[k+1], PCD, PCU),
            )
        end

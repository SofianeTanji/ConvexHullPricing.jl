...
function FastGradientMethod(instance, X0, niter, γ, smoothing_parameter, verbose = -1)
    ...
    for t = 1:niter
        ...
        fun_oracle, grad_oracle =
            Utilitaries.exact_smooth_oracle(instance, y_iterates[t], smoothing_parameter)
...
function tFastGradientMethod(instance, X0, τ, smoothing_parameter, verbose = -1)
    T = length(instance.Load)
    NbGen = length(instance.ThermalGen.MinRunCapacity)
    Lips = 1.0 # ((1 + 1 / (NbGen^2)) * T)/smoothing_parameter
    x_iterates = [X0]
    y_iterates = [X0]
    fun_iterates = Array([])
    time_vector = [0.0]
...
function tShiftedFGM(instance, X0, τ, α, smoothing_parameter, verbose = -1)
    ...
    while time_vector[end] <= τ
        ...
        it_time = @elapsed begin
            fun_oracle, grad_oracle = Utilitaries.exact_translate_smooth_oracle(
                instance,
                y_iterates[t],
                smoothing_parameter,
                shift,
            ...
            push!(
                x_iterates,
                Utilitaries.ProjBox(y_iterates[t] - α * grad_oracle, PCD, PCU),
            )
    ...
    while timev[end] <= Τ
        time_it = @elapsed begin
            ...
            while true
                ...
                fy, gy =
                    Utilitaries.exact_translate_smooth_oracle(instance, yn, μ, shift)

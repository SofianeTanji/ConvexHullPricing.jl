...
function tShiftedFastGradientMethod(instance, X0, τ, smoothing_parameter, shift, verbose = -1)
    T = length(instance.Load)
    NbGen = length(instance.ThermalGen.MinRunCapacity)
    Lips = ((1 + 1 / (NbGen^2)) * T)/smoothing_parameter
    x_iterates = [X0]
...
function tGradientMethod(instance, X0, τ, smoothing_parameter, verbose = -1)
    iterates = [X0]
    fun_iterates = Array([])
    ...
    while time_vector[end] <= τ
        ...
        push!(iterates, Utilitaries.ProjBox(iterates[i] - (1e-5) * grad_oracle, PCD, PCU))
        end
        push!(time_vector, it_time + time_vector[end])
        i += 1
    end
...
function fista(instance, X0, Τ, μ, Lmin, ρ, δ)
    τ = [1.]
    ρ, δ = ρ, δ
    x = [X0, X0]
    f = []
    ...
    while timev[end] <= Τ
        time_it = @elapsed begin
            ...
            while true
                ...
                β = (t - 1)/tn
                yn = x[k] + β * (x[k + 1] - x[k])
                fy, gy = Utilitaries.exact_smooth_oracle(instance, yn, μ)

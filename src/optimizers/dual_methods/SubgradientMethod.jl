...
function tStochasticSubgradientMethod(instance, initial_prices, budget, alpha, batchsize = 5, verbose = -1)
    iterates = [initial_prices]
    fun_iterates = Array([])
...
function tRSG(instance, initial_prices, budget, K, eta1, verbose = 1)
    ...
    while time_vector[end] <= budget
        ...
        xb, it, fb, tb = tSubgradientMethod(instance, sum(iterates)/length(iterates), budget / K, eta1)
...
function tnRSG(instance, initial_prices, budget, K, eta1, verbose = 1)
    ...
    while time_vector[end] <= budget
        ...
        xb, it, fb, tb = tnSubgradientMethod(instance, sum(iterates)/length(iterates), budget / K, eta1)
        iterates = [iterates; it]
        fun_iterates = [fun_iterates; fb]
        time_vector = [time_vector; tb[2:end] .+ time_vector[end]]
        eta1 = eta1 / 2
...
function tnSubgradientMethod(instance, initial_prices, budget, alpha, verbose = -1)
    ...
    while time_vector[end] <= budget
        ...
        push!(iterates, Utilitaries.ProjBox(iterates[i] - (alpha / (norm(grad_oracle) * i)) * grad_oracle, PCD, PCU))
        end
        push!(time_vector, it_time + time_vector[end])
        i += 1
    end

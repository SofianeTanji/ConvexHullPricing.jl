...
function BundleLevelMethod(instance, initial_prices, niter, alpha, verbose = -1)
    ...
    for i = 1:niter
        ...
        @constraint(
            model_projection,
            fun_oracle + dot(grad_oracle, ProjPrice - iterates[i]) <= LevelSet
        )
        @objective(
            model_projection,
            Min,
...
function tOptimal(instance, initial_prices, alpha, verbose = 1)
    ...
    while UpperBound - LowerBound >= 50
        ...
        it_time = @elapsed begin
            ...
            @constraint(
                model_projection,
                fun_oracle + dot(grad_oracle, ProjPrice - iterates[idx]) <= LevelSet
            )
            @objective(
                model_projection,
                Min,
                sum((ProjPrice[t] - iterates[idx][t])^2 for t = 1:T)
            )
            optimize!(model_projection)
            push!(iterates, value.(ProjPrice))
        end

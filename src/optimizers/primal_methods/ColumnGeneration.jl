...
function ColumnGeneration(instance, initial_prices, niter, eps, verbose = -1)
    ...
    for gen = 1:NbGen
        ...
        @constraint(
            ...
            VarCost - sum(
                ...
                MarginalCost[gen] * Varp[t] for t = 1:T
            ) == 0
        )
        ...
        @objective(
            ...
            sum(
                ...
                FixedCost[gen] * Varv[t] +
                MarginalCost[gen] * Varp[t] for t = 1:T
            ) - sum(price[t] * Varp[t] for t = 1:T)
        )
        subproblem = Utilitaries.SubProblem(
            model_subproblem,
            Varp,
            Varpbar,
            Varu,
    ...
    for gen = 1:NbGen
        ...
        if NbGen > 1
            ...
            ScheduleCost[gen, ScheduleCounter[gen]] = sum(
                NoLoadConsumption[gen] * MatchU[gen, t] +
                FixedCost[gen] * MatchV[gen, t] +
                MarginalCost[gen] * MatchP[gen, t] for t = 2:T+1
            )
        else
            ...
            ScheduleCost[gen, ScheduleCounter[gen]] = sum(
                NoLoadConsumption[gen] * MatchU[t] +
                FixedCost[gen] * MatchV[t] +
                MarginalCost[gen] * MatchP[t] for t = 2:T+1
            )
        end
    ...
    VarZ = @variable(
        restricted_model,
        [gen = 1:NbGen, i = 1:ScheduleCounter[gen]],
        lower_bound = 0,
        upper_bound = 1,
        base_name = "Z"
    )
    ...
    ConstrBalance = @constraint(
        restricted_model,
        [t = 1:T],
        sum(
            sum(
                sum(
                    DictZ[gen, i] * prod for
                    (time, prod) in enumerate(ScheduleP[gen, i]) if time == t
                ) for i = 1:ScheduleCounter[gen]
            ) for gen = 1:NbGen
        ) - VarL[t] == 0
    )
    ConstrConvexComb = @constraint(
        restricted_model,
        [gen = 1:NbGen],
        sum(DictZ[gen, i] for i = 1:ScheduleCounter[gen]) == 1
    )
    # set_dual_start_value(ConstrBalance, initial_prices)
    RMP = Utilitaries.RestrictedMasterProgram(
        restricted_model,
        DictZ,
        VarL,
        ConstrBalance,
    ...
    for iter = 1:niter
        ...
        @objective(
            ...
            Min,
            sum(
                sum(DictZ[gen, i] * ScheduleCost[gen, i] for i = 1:ScheduleCounter[gen]) for
                gen = 1:NbGen
        ...
        for gen = 1:NbGen
            ...
            @objective(
                ...
                Min,
                sum(
                    NoLoadConsumption[gen] * SubProblem.Varu[t] +
                    FixedCost[gen] * SubProblem.Varv[t] +
                    MarginalCost[gen] * SubProblem.Varp[t] for t = 1:T
                ) - sum(price[t] * SubProblem.Varp[t] for t = 1:T)
            )
            ...
            if ReducedCost <= -eps
                ...
                ScheduleCost[gen, ScheduleCounter[gen]] = sum(
                    NoLoadConsumption[gen] * SubU[t] +
                    FixedCost[gen] * SubV[t] +
                    MarginalCost[gen] * SubP[t] for t = 2:T+1
                )

                NewVarZ = @variable(
                    RMP.model,
                    [[gen], [ScheduleCounter[gen]]],
                    lower_bound = 0,
                    upper_bound = 1,
                    base_name = "Z"
                )
                ...
                for t = 1:T
                    set_normalized_coefficient(
                        RMP.ConstrBalance[t],
                        RMP.VarZ[gen, ScheduleCounter[gen]],
                        SubP[t+1],
                    )
                end
                set_normalized_coefficient(
                    RMP.ConstrConvexComb[gen],
                    RMP.VarZ[gen, ScheduleCounter[gen]],
                    1,
                )
            end
    ...
    while time_vector[end] <= budget
        ...
        it_time = @elapsed begin
            @objective(
                ...
                sum(
                    sum(
                        DictZ[gen, i] * ScheduleCost[gen, i] for i = 1:ScheduleCounter[gen]
                    ) for gen = 1:NbGen
                ) + LostLoad * sum(L[t] - VarL[t] for t = 1:T)
            )
            optimize!(RMP.model)
            ...
            for gen = 1:NbGen
                ...
                if ReducedCost <= eps
                    ...
                    RMP.VarZ[gen, ScheduleCounter[gen]] =
                        NewVarZ[gen, ScheduleCounter[gen]]
